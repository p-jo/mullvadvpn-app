...
fn wait_for_ready_device(alias: &str) -> Result<()> {
    ...
    unsafe {
        ...
        let first_table = &(*unicast_table).Table[0] as *const MIB_UNICASTIPADDRESS_ROW;

        for i in 0..(*unicast_table).NumEntries {
            let table = first_table.offset(i as isize);
            if (*table).InterfaceLuid.Value != luid.Value {
                continue;
            }
            unicast_rows.push(*table);
        }
...
impl From<NL_DAD_STATE> for DadStateError {
    fn from(state: NL_DAD_STATE) -> DadStateError {
        ...
        match state {
            ...
            other @ _ => DadStateError::Unknown(other),
        }
    }
}
